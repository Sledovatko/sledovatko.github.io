...
function accountError(error) {
  ...
  if (['provider_disabled', 'oauth_provider_not_supported'].includes(code) || /unsupported provider|provider is not enabled/.test(message)) return 'Přihlášení přes GitHub teď není dostupné. Zkus to později nebo pokračuj bez účtu.';
...
function accountGithubButton() {
  return Account.githubEnabled === true ? `<button type="button" class="btn btn--ghost account-oauth" data-account-github>${ACCOUNT_GITHUB_ICON}<span>Pokračovat přes GitHub</span></button>` : '';
}
...
function openAccount(mode = 'login') {
  ...
  function bindCommon() {
    ...
    body.querySelectorAll('[data-account-github]').forEach(button => button.onclick = () => run(async () => {
      if (!Account.configured || Account.githubEnabled !== true || !Account.client) throw { code: 'provider_disabled' };
      flash('Přesměrovávám na GitHub…');
      const result = await Account.client.auth.signInWithOAuth({ provider: 'github', options: { redirectTo: accountRedirect('oauth') } });
  ...
  function renderOAuthOnly() {
    setTitle('Tvůj účet');
    const available = Account.githubEnabled === true;
    body.innerHTML = `<div class="account-intro"><div class="account-emblem">${ACCOUNT_USER_ICON}</div><h2>${available ? 'Přihlas se do Sledovátka' : 'Šuplík máš pořád po ruce'}</h2><p>${available ? 'Šuplík, epizody a hodnocení s tebou na všech zařízeních.' : 'Přihlášení teď není dostupné. Sbírku můžeš dál používat v tomto prohlížeči.'}</p></div>
      ${feedback()}${accountGithubButton()}
      ${available ? '<p class="account-oauth-note">Při prvním přihlášení se ti automaticky vytvoří účet ve Sledovátku. Přihlášení potvrzuješ přímo na GitHubu.</p>' : ''}
  ...
  function renderForm() {
    ...
    body.innerHTML = `<div class="account-intro"><div class="account-emblem">${ACCOUNT_USER_ICON}</div><h2>${title}</h2><p>${updating ? 'Použij vlastní dlouhé heslo. Správce hesel ti s ním může pomoct.' : reset ? 'Pošleme ti odkaz, přes který nastavíš nové heslo.' : 'Šuplík, epizody a hodnocení s tebou na všech zařízeních.'}</p></div>
      ...
      ${!updating && !reset && Account.githubEnabled === true ? accountGithubButton() + '<div class="account-or"><span>nebo e-mailem</span></div>' : ''}
...
function showAccountPrivacy() {
  const methods = accountEmailEnabled() ? 'E-mail slouží k přihlášení, potvrzení registrace a obnově přístupu.' : Account.githubEnabled === true ? 'Přihlášení potvrzuješ na GitHubu. Identita a e-mail z profilu slouží k rozpoznání tvého účtu; heslo ke GitHubu Sledovátku nesděluješ.' : 'Přihlášení na této verzi webu zatím není dostupné.';
