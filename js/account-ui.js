...
function accountError(error) {
  ...
  if (['provider_disabled', 'oauth_provider_not_supported'].includes(code) || /unsupported provider|provider is not enabled/.test(message)) return 'Přihlášení teď není dostupné. Zkus to později nebo pokračuj bez účtu.';
  if (code === 'manual_linking_disabled') return 'Připojení dalšího přihlášení teď není dostupné. Tvůj současný účet funguje dál.';
  if (['identity_already_exists', 'email_conflict_identity_not_deletable'].includes(code)) return 'Tento Google účet už patří jinému účtu Sledovátka. Použij Google účet spojený se svojí sbírkou.';
...
function accountOAuthButtons() {
  const buttons = Account.googleEnabled === true ? `<button type="button" class="account-oauth account-oauth--google" data-account-oauth="google">${ACCOUNT_GOOGLE_ICON}<span>Pokračovat přes Google</span></button>` : '';
...
function accountIdentityProviders() {
  return [...new Set([...(Account.user?.identities || []).map(identity => identity.provider), ...(Account.user?.app_metadata?.providers || [])])]
    .filter(provider => provider === 'google' && Account.googleEnabled === true || provider === 'email' && accountEmailEnabled());
}
...
function openAccount(mode = 'login') {
  ...
  function bindCommon() {
    ...
    body.querySelectorAll('[data-account-oauth]').forEach(button => button.onclick = () => run(async () => {
      ...
      flash('Otevírám přihlášení Google…');
  ...
  function renderOAuthOnly() {
    setTitle('Tvůj účet');
    const available = Account.googleEnabled === true;
  ...
  function renderForm() {
    ...
    body.innerHTML = `<div class="account-intro"><div class="account-emblem">${ACCOUNT_USER_ICON}</div><h2>${title}</h2><p>${updating ? 'Použij vlastní dlouhé heslo. Správce hesel ti s ním může pomoct.' : reset ? 'Pošleme ti odkaz, přes který nastavíš nové heslo.' : 'Šuplík, epizody a hodnocení s tebou na všech zařízeních.'}</p></div>
      ...
      ${!updating && !reset && Account.googleEnabled === true ? accountOAuthButtons() + '<div class="account-or"><span>nebo e-mailem</span></div>' : ''}
  ...
  function renderSignedIn() {
    ...
    body.innerHTML = `<div class="account-intro account-intro--member"><div class="account-emblem">${ACCOUNT_USER_ICON}</div><h2>Všechno na svém místě</h2><p class="account-email">${escHtml(Account.user.email || '')}</p></div>
      ...
      ${identities.length ? `<p class="account-connected">Připojené přihlášení: <strong>${identities.map(provider => ({ google: 'Google', email: 'E-mail' }[provider])).join(', ')}</strong></p>` : ''}
      ${canLinkGoogle ? `<button type="button" class="account-choice account-link-google" data-account-link-google><strong>Připojit Google k tomuto účtu</strong><span>Ke své sbírce se pak přihlásíš přes Google. Účet a uložené tituly zůstanou stejné.</span></button>` : ''}
...
function showAccountPrivacy() {
  const providers = Account.googleEnabled === true ? 'Google' : '';
  const methods = (providers ? 'Přihlášení potvrzuješ přímo u Googlu. Identita a e-mail z profilu rozpoznají tvůj účet; heslo poskytovatele Sledovátku nesděluješ. ' : '') + (accountEmailEnabled() ? 'E-mail slouží také k přihlášení, potvrzení registrace a obnově přístupu.' : !providers ? 'Přihlášení na této verzi webu zatím není dostupné.' : '');
